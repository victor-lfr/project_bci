...
#include <koala_driver.h>
#include <stdexcept>
#include <functional>

void decode_command(const geometry_msgs::TwistConstPtr& wheel_speed_msg, const koala::WheelSpeed& client) {
...
int main(int argc, char* argv[]) {
  ...
  koala::WheelServer wheel_server;

  // Definition of the koala_driver nodes
  koala::IRReader ir_reader(wheel_server);
  koala::WheelSpeed speed_client(wheel_server);
  ...
  std::array<int, koala::IRReader::nb_ir_sensors> ir_raw;
  std::array<double, koala::IRReader::nb_ir_sensors> ir_ranges;
  ...
  msg_ir.layout.dim[0].size = koala::IRReader::nb_ir_sensors;
  msg_ir.layout.dim[0].stride = koala::IRReader::nb_ir_sensors;
  msg_ir.data.resize(koala::IRReader::nb_ir_sensors);

  std_msgs::Int32MultiArray msg_ir_raw;
  msg_ir_raw.layout = msg_ir.layout;
  msg_ir_raw.data.resize(koala::IRReader::nb_ir_sensors);
